...
class MotorCalibrationPlotter:
    def __init__(self, port='COM4', baudrate=115200, log_dir='motor_logs'):
    ...
    def calculate_linear_approximation(self):
        ...
        approx_results = []
        for velocity_data, motor_name in motor_data:
            # Positive percentages
            pos_x = np.array(self.percentages)[pos_mask]
            pos_y = np.array(velocity_data)[pos_mask]
            if len(pos_x) > 1:
            ...
            neg_y = np.array(velocity_data)[neg_mask]
            if len(neg_x) > 1:
    ...
    def update_plot(self, frame):
        if self.serial.in_waiting:
            ...
            if data:
                percentage, lv, rv = data
                
                # Store the data
                self.percentages.append(percentage)
                self.left_velocity.append(abs(lv))
                self.right_velocity.append(abs(rv))
                ...
                # Update axis limits
                all_rps = self.left_velocity + self.right_velocity
                if all_rps:
                    max_rps = max(all_rps)
    ...
    def calculate_inverse_functions(self, approx_results):
        ...
        fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(10, 10))
        fig.suptitle('Motor Inverse Functions (Velocity to Percentage)')
        ...
        def get_inverse_function(slope, intercept):
            return lambda velocity: (velocity - intercept)/slope
        ...
        for i, result in enumerate(pos_results):
            ...
            pos_mask = np.array(self.percentages) >= 0
            velocity_data = velocity_data[pos_mask]
            
            if len(velocity_data) > 0:
                velocity_range = np.linspace(0, max(velocity_data), 100)
                percentage_range = [inverse_func(velocity) for velocity in velocity_range]
                
                # Plot inverse function with simplified equation
                ax1.plot(velocity_range, percentage_range, c=colors[i], linestyle='--',
                        label=f'{motor}: y = {1/slope:.4f}x + {-intercept/slope:.4f}')
                
                # Plot original data points
                ax1.scatter(velocity_data, np.array(self.percentages)[pos_mask], 
        ...
        ax2.set_title('Negative Velocity to Percentage')
        for i, result in enumerate(neg_results):
            ...
            rps_data = {
                'Left ': self.left_velocity,
                'Right Back': self.right_velocity
            }[motor]
            rps_data = np.array(rps_data)
            
            neg_mask = np.array(self.percentages) < 0
            velocity_data = velocity_data[neg_mask]
            
            if len(velocity_data) > 0:
                rps_range = np.linspace(0, max(velocity_data), 100)
                percentage_range = [inverse_func(velocity) for velocity in velocity_range]
                
                # Plot inverse function with simplified equation
                ax2.plot(velocity_range, percentage_range, c=colors[i], linestyle='--',
                        label=f'{motor}: y = {1/slope:.4f}x + {-intercept/slope:.4f}')
                
                # Plot original data points
                ax2.scatter(velocity_data, np.array(self.percentages)[neg_mask], 
                        c=colors[i], alpha=0.5)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--port', default='COM4', help='Serial port to use')
